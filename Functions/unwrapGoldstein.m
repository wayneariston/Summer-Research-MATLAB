function im_unwrapped = unwrapGoldstein(image)
    %% Replace with your images
    im_mag   = abs(image);                  %Magnitude image
    im_phase = angle(image);                %Phase image
    
    %% Replace with your mask (if desired)
    im_mask = ones(size(image));
    mag_max = max(im_mag(:));
    ...
    if(~exist('im_mask','var'))
      im_mask = ones(size(image));          %Mask (if applicable)
    end
...
%     branch_cuts = zeros(size(im_phase));
